 ...
 */
function updateEventParticipantCount(eventCode) {
  const allRegistrations = getSheetData(SHEETS_CONFIG.SHEETS.REGISTRATIONS);
  const newTotal = allRegistrations.filter(row => row[1] === eventCode).length;
  const eventRowIndex = findRowIndexByValue(SHEETS_CONFIG.SHEETS.EVENTS, 1, eventCode);
  if (eventRowIndex !== -1) {
    setCellValue(SHEETS_CONFIG.SHEETS.EVENTS, eventRowIndex, 10, newTotal);
  }
}
...
function isEventOpen(eventCode) {
  const event = findRowByValue(SHEETS_CONFIG.SHEETS.EVENTS, 1, eventCode);
...
function registerToEvent(userId, displayName, messageText) {
  ...
  if (!isEventOpen(eventCode)) return ERROR_MESSAGES.EVENT_CLOSED(eventCode);
  ...
  const totalParticipants = 1 + additionalCount;
  const now = Utilities.formatDate(new Date(), USER_CONFIG.TIMEZONE, USER_CONFIG.DATE_FORMAT);

  const eventRegistrations = filterRowsByValue(SHEETS_CONFIG.SHEETS.REGISTRATIONS, 1, eventCode);
  const newNames = Array.from({ length: totalParticipants }, (_, i) => i === 0 ? baseName : `${baseName}${i + 1}`);

  for (const name of newNames) {
    if (eventRegistrations.some(row => row[2] === userId && row[3] === name)) {
      return ERROR_MESSAGES.ALREADY_REGISTERED(name, eventCode);
    }
  }

  let maxOrder = eventRegistrations.length > 0 ? Math.max(...eventRegistrations.map(row => parseInt(row[4] || 0, 10))) : 0;

  const rowsToAdd = newNames.map((name, i) => {
    const registerId = `R${new Date().getTime()}${i}`;
    return [registerId, eventCode, userId, name, ++maxOrder, remark.trim(), now];
  });

  rowsToAdd.forEach(row => appendRow(SHEETS_CONFIG.SHEETS.REGISTRATIONS, row));
  updateEventParticipantCount(eventCode);
...
function getRegistrationList(userMessage) {
  ...
  const [, , groupId, eventDateRaw, , timeRange, arenaCode, address] = event;
  ...
  const eventRegs = filterRowsByValue(SHEETS_CONFIG.SHEETS.REGISTRATIONS, 1, eventCode)
...
function reorderRegistrations(eventCode) {
  ...
  for (let i = 0; i < eventRegs.length; i++) {
    regSheet.getRange(eventRegs[i].rowIndex, 5).setValue(i + 1);
  }
}
